 ...
 * tous les controlleurs Herite de cette classe, utilis�e pour regroup�e des attributs et des methodes.
 */
	protected int sauv;

	protected  Composant cmp; //utilis� si un composant est selectionn� 
	protected static ClickDroitFil clickDroitFilFenetre; 
	protected static Polyline lineDroit;  //utilis� si un fil (Polyline)  est selectionn�
	...
	protected static Line guideFilX = new Line(); //Les guides du fil (du tra�age) 
	...
	protected static ArrayList<Pin> ListTextPin = null; //la liste des pins quand l'utilisateur selectionne l'ordre des entr�es (table de verite ou circuit personnalis��)
	protected static ArrayList<Text> ListText = null;   //la liste des numeros quand l'utilisateur selectionne l'ordre des entr�es (table de verite ou circuit personnalis��)
	...
	protected int entree; // le numero de l'entr�e
	protected int sortie; // le numero de la sortie
	protected int rel;

	protected static ArrayList<Button> rightBareButtons = new ArrayList<Button>() ; //pour les desactiv�s en mode de creation
	
	protected Circle relieCercle ; //quand un fil est reli� , un cercle est gener� au niveau du l'entr�e
	...
	//la fonction responsable de la gestion des fils : 
		 ...
		 * de la position de la souris avec un certain traitement sp�cefique . 
		 */
		 ...
		 * quand on clic sur un polyline ,un autre polyline est cre�,ajout� au workspace et ajout�
		...
		/*Fonctionnement :"La relation " entre composants ,Si le polyline est relach� dans un autre composant
		 * alors: 
		 * 		Si relach� dans une entr� ,une relation se fait entre le composant source du polyline ,et ce dernier .
		 * 		Sinon : le polyline est supprim� .
		 */
			@Override
				...
				if (!simul) {
					if (arg0.getButton() == MouseButton.PRIMARY) {
						...
						if (( destination = intersectionFilComposants(arg0.getX(), arg0.getY())) != null) {
							/*rel : si 0 n'est pas reli� a une entr�e ( polyline � supprim� ) ,
							*		si 1 
							*/

							//Polyline n'est pas relach� dans un composant .
							if (rel == 0) {
								// suppression du fil
	...
	/*Role : Pour avoir si un fil est relach� dans l'un des composants du Workspace 
	 */
		Composant cmp = null;
		boolean trouv = false;
	...
	public int intersectionFilComposant(ImageView imgCmp, double Xfil, double Yfil) {
		/*Role : Pour avoir si un fil est relach� dans l'espace d'un composant ou pas 
		 */
		...
		double ci = 0; 
		if(imgCmp.getId().equals("CircuitIntegre") || imgCmp.getId().equals("CircuitIntegreSequentiel")) //un traitement pour les coordonn�es des circuit integr�es
			ci = 5;
	...
	/*Role : creer une cercle quand un fil est reli� */
