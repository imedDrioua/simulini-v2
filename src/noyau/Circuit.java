...
public class Circuit {
	...
	private static ArrayList<Composant> composantsErronee = new ArrayList<Composant>();// la liste des composants erron�s

	
	public static void ajouterCompErrone(Composant composant) { /// pour ajouter des composant erronn�s
		composantsErronee.add(composant);
	}
	
	public static void AjouterListeException(ArrayList<ExceptionProgramme> exceptionProgramme) { /// ajouter une liste d'exception � la liste des exceptions du circuit
	...
	public static void validerCircuits() { /// elle est utilis�e pour vous dire es que un circuit est valid� ou non
	...
	public static void ajouterComposant(Composant comp,ImageView img) { // ajouter un composant � la liste des composants utilis�s
		compUtilises.put(comp, img);
	}
	public static void ajouterFil(Fil fil,ArrayList<InfoPolyline> polyline) { // ajouter un fil � la liste des fils 
		filUtilises.put(fil, polyline);
	}
	public static void ajouterEntree(Pin pin) { // ajouter une entree � la liste des entrees
		entreesCircuit.add(pin);
		
	}
	public static void ajouterSourceCte(SourceConstante cte) { // ajouter une entree � la liste des entrees
		listSouceCte.add(cte);
	}
	public static void supprimerSourceCte(SourceConstante cte) {
		listSouceCte.remove(cte);
	}
	public static void ajouterSortie(Pin compAff) { // ajouter une sorties � la liste des sorties
	...
	public static void relierHorloge(Sequentiels composant,Composant horloge,int sortie) { // pour relier une horloge pour les elts sequentiels
		...
		if (! listeEtages.contains(composant)) { // ajouter le composant � la liste des �tages si il ne se trouve pas
	...
	public static void tableVerite(ArrayList<Pin> entreesCircuit,ArrayList<Pin> sortiesCircuit) // generer la table de verit� du ciruit
	{
		...
		int c= nbEntrees+nbSorties; // la taille de la table de verit�
		int j,k=0;
		double l=Math.pow(2, nbEntrees); // calculer le nombre de lignes de la table de verit�
		tableVerite = new EtatLogique[(int)l][c]; // initialiser la taille de la table
		EtatLogique ligne[] ; // sert � stocker une ligne 
		...
		for(int i=0;i<l;i++)
		{
			...
			tableVerite[i]=Arrays.copyOf(ligne, c); // inserer la ligne dans la table de verit�
		}
	}
	
	public static void afficher() { // pour l'affichage en mode console de la table de v�rit� 
	...
	public static void initialiser() {// � completer au fur et mesure .
		...
		int max;
		for (int i = 0; i < listeEtages.size(); i++) { // former les �tages du circuit pour l'execution et la generation du chronogramme
	...
	public static ImageView getImageFromComp(Composant comp) { // recuperer l'image associ� � un composant .
		return compUtilises.get(comp);
	}
	public static Composant getCompFromImage(ImageView img) { // recuperer le composant associ� � une image .
	...
	public static ArrayList<InfoPolyline> getPolylineFromFil(Fil fil) { // recuprer les infos polyline li�e � un fil .
		return filUtilises.get(fil);
	}
	public static Fil getFilFromPolyline(Polyline ligne) { // recuperer le composant associ� � une image .
		Fil fil = null;
	...
	public static void defaultCompValue() {/// affecter les valeurs par defaut � tout les composants
	...
	public static ArrayList<InfoPolyline> getListFromPolyline(Polyline ligne) { // recuperer le composant associ� � une image .
		for (Entry<Fil, ArrayList<InfoPolyline>> entry : filUtilises.entrySet()) {
			if (entry.getValue().contains(new InfoPolyline(ligne))) {
	...
	public static void removeCompFromImage(ImageView img) { // recuperer le composant associ� � une image .
	...
	public static ArrayList<Polyline> supprimerComp(Composant composant) { /// utiliser dans la suppression des composants
		...
		for (int i = 0; i < composant.getNombreSortie(); i++) { /// derelier les composants qui sont au tour du composant � supprimer
