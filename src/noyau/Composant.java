...
public abstract class Composant implements Serializable{
	...
	public  void derelierComp() { // pour derelier le composant de ces fils d'entrees  (le composant � supprimer)
		for (int i = 0; i < nombreEntree; i++) {
			if (entrees[i] != null) {
				if (! entrees[i].getSource().equals(this)) { // pour savoir si une entree est reli� avec une sortie du mm composant
	...
	public boolean isDessocier() { /// retourne vrai si le composant est derelier
		...
		int i = 0;
		while(( i < nombreEntree) && (dessocier == true)) { /// savoir si les entrees de ce composants sont reli�es
			if (entrees[i] != null) {
				dessocier = false;
			}
			else i++;
		...
		i = 0;
		while((i < nombreSortie) && (dessocier == true) ) /// /// savoir si les sorties de ce composants sont reli�es
		{
	...
	public void validerComposant() { /// utiliser pour detecter les erreurs relative � un composant donn�
		// TODO Auto-generated method stub
		...
		if (arrayList.size() == nombreEntree) { /// si toutes les entrees sont dereli�es
	...
	public void setNombreSortieAndUpdateFil(int nombreSortie) { /// mettre � jour le nombre de sortie et generer les fils n�cessaires
